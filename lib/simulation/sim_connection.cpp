...
const sim_connection::params dsl15_dn = {DSL_DN_BW * 1024 / 8,
                                         txdelay(DSL_RTDELAY * 1000 / 2),
                                         txdelay(DSL_DN_QLEN * 1000),
                                         0.0};
const sim_connection::params dsl15_up = {DSL_UP_BW * 1024 / 8,
                                         txdelay(DSL_RTDELAY * 1000 / 2),
                                         txdelay(DSL_UP_QLEN * 1000),
                                         0.0};

// Typical residential cable modem link
const sim_connection::params cable5_dn = {CABLE_DN_BW * 1024 / 8,
                                          txdelay(CABLE_RTDELAY * 1000 / 2),
                                          txdelay(CABLE_DN_QLEN * 1000),
                                          0.0};
const sim_connection::params cable5_up = {CABLE_UP_BW * 1024 / 8,
                                          txdelay(CABLE_RTDELAY * 1000 / 2),
                                          txdelay(CABLE_UP_QLEN * 1000),
                                          0.0};
...
const sim_connection::params eth10 = {ETH10_RATE,
                                      txdelay(ETH10_DELAY / 2),
                                      txtime(ETH_QBYTES, ETH10_RATE),
                                      0.0};
const sim_connection::params eth100 = {ETH100_RATE,
                                       txdelay(ETH100_DELAY / 2),
                                       txtime(ETH_QBYTES, ETH100_RATE),
                                       0.0};
const sim_connection::params eth1000 = {ETH1000_RATE,
                                        txdelay(ETH1000_DELAY / 2),
                                        txtime(ETH_QBYTES, ETH1000_RATE),
                                        0.0};

// Satellite link parameters (XXX need to check)
const sim_connection::params sat10 = {ETH10_RATE, txdelay(500000), txdelay(1024 * 1024), 0.0};

// All the values below are a wild guess!

#define WIFI_MTU 1500 // Standard Wifi MTU
...
const sim_connection::params wifi54 = {WIFI54_RATE,
                                       txdelay(500 / 2),
                                       txtime(WIFI_QBYTES, WIFI54_RATE),
                                       0.05};

const sim_connection::params wifi600 = {WIFI600_RATE,
                                        txdelay(100 / 2),
                                        txtime(WIFI_QBYTES, WIFI600_RATE),
                                        0.07};
...
{
    string speed = boost::str(rate < 1024 * 1024 ?
                                  boost::format("%fKbps") % (float(rate * 8) / 1024) :
                                  boost::format("%fMbps") % (float(rate * 8) / (1024 * 1024)));
    return boost::str(boost::format("%s, delay %sms, qlen %sms (%.3f loss)") % speed
...
void
sim_connection::connect(std::shared_ptr<sim_host> downlink,
                        uia::comm::endpoint downlink_address,
                        std::shared_ptr<sim_host> uplink,
                        uia::comm::endpoint uplink_address)
{
...
void
sim_connection::set_preset(preset p)
{
    switch (p) {
        case dsl_15: return set_link_params(dsl15_dn, dsl15_up);
        case cable_5: return set_link_params(cable5_dn, cable5_up);
        case sat_10: return set_link_params(sat10);
        case eth_10: return set_link_params(eth10);
        case eth_100: return set_link_params(eth100);
        case eth_1000: return set_link_params(eth1000);
...
{
    if (link == downlink_)
        return downlink_address_;
    if (link == uplink_)
        return uplink_address_;
...
{
    if (host == downlink_)
        return downlink_params_;
    if (host == uplink_)
        return uplink_params_;
...
{
    if (host == downlink_)
        return downlink_arrival_time_;
    if (host == uplink_)
        return uplink_arrival_time_;
    throw std::runtime_error("Target host not on this connection!");
}
